function circle(x,y,r,holePt){
	...
	return {
		...
		update: function(){
			...
			//other balls
			if(Math.abs(that.dx)<.1 && Math.abs(that.dy)<.1)
				{
				that.dx = 0;
				that.dy = 0;
			}
			that.dx = that.dx*.98
			that.dy = that.dy*.98
			that.x = that.x+that.dx*.9
			that.y = that.y+that.dy*.9
			...
			distanceFromHole = that.shape.bounds.center.subtract(hole.bounds.center).length
			that.shape.fillColor = colorWheel(cc)
			cc+=2
			if(distanceFromHole<10 && (Math.abs(that.dx)+Math.abs(that.dy)/2<10))
				{
				that.shape.remove()
				inthehole.play()
				return 'new'
...
function hitAWall(wallList,that,v){
	for(var k=0;k<wallList.length;k++){ //HIT A WALL?
				wall = wallList[k]
...
function hitAHazard(hazardList,that){
	v = new paper.Point(that.dx,that.dy)
	var n;
	for(var k=0;k<hazardList.length;k++){ //HIT A SHAPE?
				hazard = hazardList[k]
